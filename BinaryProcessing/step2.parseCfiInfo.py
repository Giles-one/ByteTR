...
from elftools.dwarf.locationlists import LocationParser, LocationEntry, LocationViewPair, \
    BaseAddressEntry as LocBaseAddressEntry, LocationListsPair
...
class ReadElf(object):
            ...
            for entry in lineprogram.get_entries():
                ...
                if state is None:
                    ...
                    self._emitline('%-35s  %s  %18s%s %s' % (
                        ...
                        '0' if state.address == 0 else self._format_hex(state.address),
                        '' if lineprogram.header.maximum_operations_per_instruction == 1 else '[%d]' % (
                        state.op_index,),
                        'x' if state.is_stmt and not state.end_sequence else ''))
    ...
    def _dump_loclist(self, loc_list, line_template, cu_map):
        ...
        for entry in loc_list:
            if isinstance(entry, LocationViewPair):
                ...
                self._emitline(
                    "    %08x v%015x v%015x location view pair" % (entry.entry_offset, entry.begin, entry.end))
            else:
                if in_views:
                    in_views = False
                    self._emitline("")
        ...
        for range_list in range_lists:
            ...
            if ver5 and range_list[0].entry_offset > next_rcu_offset:
                while range_list[0].entry_offset > next_rcu_offset:
                    ...
                    rcu_index += 1
                self._emitline('    Offset   Begin    End')
            self._dump_rangelist(range_list, cu_map, ver5, line_template, base_template, base_template_indexed,
                                 range_lists_sec)

        # TODO: trailing empty CUs, if any?

    def _dump_rangelist(self, range_list, cu_map, ver5, line_template, base_template, base_template_indexed,
                        range_lists_sec):
        ...
        last = range_list[-1]
        self._emitline(
            '    %08x <End of list>' % (last.entry_offset + last.entry_length if ver5 else first.entry_offset))
    ...
    def collectCallFrameInfo(self):
        ...
        if self._dwarfinfo.has_EH_CFI():
            collect = self._custom_dump_frames_interp_info(
                self._dwarfinfo.eh_frame_sec,
                self._dwarfinfo.EH_CFI_entries())
        if collect:
            return collect
        ...
        if self._dwarfinfo.has_CFI():
            return self._custom_dump_frames_interp_info(
                self._dwarfinfo.debug_frame_sec,
                self._dwarfinfo.CFI_entries())

    def _custom_dump_frames_interp_info(self, section, cfi_entries):
        """ Dump interpreted (decoded) frame information in a section.
        ...
        for entry in cfi_entries:
            ...
            lowPC  = entry['initial_location']
            highPC = entry['initial_location'] + entry['address_range']
            ...
            def custom_describe_reg_name(regnum, machine_arch):
                if machine_arch == 'x86':
                    return publicConfig['REG']['x86_32'][regnum]
                elif machine_arch == 'x64':
                    return publicConfig['REG']['x86_64'][regnum]
                elif machine_arch == 'AArch64':
                    return publicConfig['REG']['aarch64'][regnum]
                elif machine_arch == 'ARM':
                    return publicConfig['REG']['arm32'][regnum]
                elif machine_arch == 'MIPS':
                    return publicConfig['REG']['mips'][regnum]
                else:
                    return None
            ...
            for line in decoded_table.table:
                if line['cfa'] is None:
                    continue
                if line['cfa'].expr is not None:
                    continue
                
                # regName = describe_reg_name(line['cfa'].reg, self.elffile.get_machine_arch())
                # print(regName)
                regName = custom_describe_reg_name(line['cfa'].reg, self.elffile.get_machine_arch())
                # print(regName)
                items.append({
                    'pc'  : line['pc'],
                    'cfa' : {
                        'reg' : regName,
                        'offset' : line['cfa'].offset
                    }
                })
            collectCFI.append({
                'lowPC': lowPC,
...
def process_ELF(ELFPath, stream=None):
    with open(ELFPath, 'rb') as file:
        readelf = ReadElf(file, stream or sys.stdout)
        collectCFI = readelf.collectCallFrameInfo()
        if not collectCFI:
            return

        global funcionFrameInfoSuffix
        saveTo = ELFPath + funcionFrameInfoSuffix
        with open(saveTo, 'w') as fp:
            json.dump(collectCFI, fp, indent=2)
        logger.info('Save to %s' % (saveTo))
...
def check():
    for root, dirs, files in os.walk('dataset'):
        for file in files:
            ...
            VarELFPath = ELFPath + SAVE_SUFFIX
            if not os.path.exists(StripELFPath) or not os.path.exists(VarELFPath):
                print(ELFPath)
