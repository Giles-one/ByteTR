...
class ReachingDefinitions(dict):
    ...
    # def __init__(self, ircfg):
        ...
        # self.ircfg = ircfg
        # self.compute()
    def __init__(self, ircfg, **kwargs):
        super(ReachingDefinitions, self).__init__()
        self.ircfg = ircfg
        if 'callArguments' in kwargs.keys():
            self.callArguments = kwargs.get('callArguments')
        if 'callerPreserved' in kwargs.keys():
            self.callerPreserved = kwargs.get('callerPreserved')
        self.compute()
    ...
    def process_block(self, block):
        ...
        # return modified
        if not self.ircfg.predecessors(block.loc_key):
            if hasattr(self, 'callArguments'):
                parameter_regs = self.callArguments
                predecessor_state = {ExprId(reg64, 64): set([(block.loc_key, -1)]) for reg64 in parameter_regs}
            else:
                predecessor_state = {}
        else:
            predecessor_state = {}
    ...
    def process_assignblock(self, block, assignblk_index):
        ...
        if hasattr(self, 'callerPreserved'):
            callee_saved_regs = self.callerPreserved
            callee_saved = list(ExprId(reg64, 64) for reg64 in callee_saved_regs)
        else:
            callee_saved = []
        ...
        for lval in assignblk:
            ...
            if rexpr.is_function_call():
                newdef = dict()
                for defval in defs:
                    if isinstance(defval, ExprId) and defval not in callee_saved:
                        continue
                    newdef[defval] = defs[defval]
                defs = newdef
            defs.update({lval: set([(block.loc_key, assignblk_index)])})
