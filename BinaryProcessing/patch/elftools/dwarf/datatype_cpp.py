...
from ..dwarf.enums import ENUM_DW_ATE
...
def custom_describe_cpp_datatype(var_die):
    t = parse_cpp_datatype(var_die)
    return (t.name, t.basetag)


def _custom_describe_datatype(var_die):
    ...
    Actually we don't care fuck cpp
    """
    ...
    while True:
        typechain.append(type_die_iter.tag)
        if type_die_iter.attributes.get('DW_AT_type'):
            type_die_iter = type_die_iter.get_DIE_from_attribute('DW_AT_type')
            continue

        type_name = bytes2str(
            type_die_iter.attributes['DW_AT_name'].value) if 'DW_AT_name' in type_die_iter.attributes else ''
        type_size = type_die_iter.attributes[
            'DW_AT_byte_size'].value if 'DW_AT_byte_size' in type_die_iter.attributes else -1
        ...
        typechain.append('%s@%d@%s' % (type_name, type_size, type_encoding[7:]))
        return typechain
