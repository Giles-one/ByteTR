...
from elftools.dwarf.datatype_cpp import DIE_name, describe_cpp_datatype, _custom_describe_datatype
...
# =========================================
    ...
    def get_formal_parameter_type(self, die: elftools.dwarf.die.DIE):
        ...
        for attr_name, attr in die.attributes.items():
            ...
            # if attr_name == 'DW_AT_type':
            #     breakpoint()
            attr_decr = self.custom_describe_attr_value(die, attr)
            variable_attrs[attr_name] = attr_decr
    ...
    def hanle_subprogram_die(self, die: elftools.dwarf.die.DIE):
        ...
        #     if isinstance(attr_name, str)
        #     else "DW_AT_unknown_%x" % (attr_name,)
        #     : self.custom_describe_attr_value(die, attr)
        #     for attr_name, attr in die.attributes.items()
        # }
        ...
        function_info = {
            ...
            'function_attr': subprogram_attrs,
            'variable': [],
            'parameter': [],
        }
    ...
    def get_data_type_info(self):
        ...
        for cu in self._dwarfinfo.iter_CUs():
            ...
            if not language in ('DW_LANG_C89', 'DW_LANG_C', 'DW_LANG_C99', 'DW_LANG_C11', 'DW_LANG_C17'):
                continue
            ...
            for child in top_die.iter_children():
                ...
                if not result:
                    continue
                collect_ELF_info.append(result)
        
        return collect_ELF_info
...
def process_ELF(filepath, stream = None):
    ...
    with open(filepath, 'rb') as file:
        ...
        result = readelf.get_data_type_info()
    ...
    saveTo = filepath + variableInfoSuffix
    with open(saveTo, 'w') as fp:
        json.dump(ELF_data, fp, indent=2)
