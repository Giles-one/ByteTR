...
def collectMemberReference(cfunc):
    if cfunc is None:
        return []
    ...
    for idx, item in enumerate(CFuncGraph.items):
        if item.op == ida_hexrays.cot_memref:
            memrefNodes.append((idx, item))
        elif item.op == ida_hexrays.cot_memptr:
            memptrNodes.append((idx, item))

    def parseMemberNode(memberNode):
        typeName = ida_hexrays.get_ctype_name(memberNode.op)
        memberOffset = memberNode.m
        memberType = memberNode.type._print()
        memberSize = memberNode.type.get_size()
        return {
            'typeName': typeName,
    ...
    def _get_expr_name(expr):
        name = expr.print1(None)
        name = ida_lines.tag_remove(name)
        name = ida_pro.str2user(name)
        return name

    def parseObjectNode(objectNode):
        expr = objectNode.cexpr
        typeName = ida_hexrays.get_ctype_name(expr.op)
        ...
        if op in [
                ida_hexrays.cot_obj,
                ida_hexrays.cot_var]:
            objectName = _get_expr_name(expr)

        objectType = objectNode.type._print()
    ...
    memUseCollect = list()
    for idx, memberNode in memrefNodes + memptrNodes:
        children = CFuncGraph.succs[idx]
        if len(children) == 0:
            print('No children')
            continue
        objectNode = CFuncGraph.items[children[0]]
        memberInfo = parseMemberNode(memberNode)
        objectInfo = parseObjectNode(objectNode)
        ...
        print(info)
        memUseCollect.append(info)
    return memUseCollect
...
for funcEA in funcs:
    ...
    memberReferenceInfo = collectMemberReference(cfunc)
    collectInfo.append({
        'functionName': funcName,
        'functionStart': funcStart,
        'memberReference': memberReferenceInfo
    })
